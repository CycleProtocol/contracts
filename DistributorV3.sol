...
/**
 * @title Distributor V3
 * @dev Cycle vault reward distribution logic
 */
contract DistributorV3 is Ownable, Pausable, ReentrancyGuard {
     ...
     */
    function _processKickback(uint256 balanceCYCLE) internal {
        ...
        // Enforce at least double the gas cost equivalent of CYCLE in the contract
        require(balanceCYCLE > amountCYCLEtoSwap.mul(2), "DistributorV3: Minimum CYCLE amount not met");
        ...
        require(success, "DistributorV3: Unable to transfer AVAX");
    }
    ...
    function distribute() external whenNotPaused nonReentrant {
        require(!Address.isContract(msg.sender), "DistributorV3: Caller is not an EOA");

        uint256 amountToDistribute = cycleBalance();
        require(amountToDistribute > 0, "DistributorV3: No CYCLE to distribute");

        _processKickback(amountToDistribute);
